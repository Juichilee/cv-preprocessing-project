...
def _dft_matrix(n):
    ...
    if not isinstance(n, np.ndarray):
        raise ValueError("dft: input must be a numpy array")
    if n.ndim != 1:
...
def _idft_matrix(n):
    ...
    """
    if not isinstance(n, np.ndarray):
        raise ValueError("idft: input must be a numpy array")
    if n.ndim != 1:
...
def dft2(img):
    ...
    dft_mat_rows = _dft_matrix(m_range)
    for i in range(N):
        dft_rows[:, i] = np.dot(dft_mat_rows, img[:, i])
    
    # Apply DFT to columns
    dft_result = np.zeros((M, N), dtype=np.complex128)
    dft_mat_cols = _dft_matrix(n_range)
    for i in range(M):
...
def idft2(F):
    ...
    idft_mat_rows = _idft_matrix(m_range)
    for i in range(N):
        idft_rows[:, i] = np.dot(idft_mat_rows, F_shifted[:, i])
    
    # Apply IDFT to columns
    idft_result = np.zeros((M, N), dtype=np.complex128)
    idft_mat_cols = _idft_matrix(n_range)
    for i in range(M):
...
def compress_image_fft(img_bgr, threshold_percentage):
    ...
    # Replace np.fft.fft2 with custom dft2
    ...
    # Replace np.fft.ifft2 with custom idft2
    ...
    # For spectrum visualization, we'll use the already shifted F from dft2
...
def low_pass_filter(img_bgr, radius):
    ...
    F_all = np.zeros((H, W, 3), dtype=np.complex128)
    for ch in range(3):
        # dft2 already returns shifted spectrum
        F_all[:, :, ch] = dft2(img[:, :, ch])

    # Apply mask (F_all is already shifted from dft2)
    mask_3c = mask[:, :, None]
