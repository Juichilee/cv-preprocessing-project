import numpy as np
# Added import for caching (required for @lru_cache)
from functools import lru_cache

# Updated: Uncommented @lru_cache, fixed validation, and implemented the function
@lru_cache(maxsize=8)
def _dft_matrix(n):
    """
    Build an n×n DFT matrix.
    """
    # Fixed: Proper validation for n as an integer (original check for ndim was incorrect)
    if not isinstance(n, int) or n <= 0:
        raise ValueError("dft_matrix: n must be a positive integer")
    
    # Implementation: Create DFT matrix where W[j, k] = e^{-2πi j k / n} / sqrt(n)
...
def _idft_matrix(n):
    ...
    # Implementation: IDFT is conjugate of DFT matrix, scaled by 1/n for consistency with np.fft.ifft
    dft_mat = _dft_matrix(n)
    return np.conj(dft_mat) / n * np.sqrt(n)  # Adjust scaling to match 1/n for IDFT

# Updated: Implemented the function with proper validation
def dft(x):
    """
...
def dft2(img):
    ...
    # Apply 1D DFT along rows, then columns
    dft_rows = np.apply_along_axis(dft, axis=1, arr=img)
    dft_2d = np.apply_along_axis(dft, axis=0, arr=dft_rows)
    # Center the zero-frequency (equivalent to fftshift)
    return np.fft.fftshift(dft_2d)  # Use np.fft.fftshift for shifting only (no FFT computation)
...
def idft2(F):
    ...
    # Unshift the centered spectrum (equivalent to ifftshift)
    F_unshifted = np.fft.ifftshift(F)  # Use np.fft.ifftshift for shifting only
    # Apply 1D IDFT along rows, then columns
...
def compress_image_fft(img_bgr, threshold_percentage):
    ...
    compressed_F = np.zeros_like(img, dtype=np.complex128)  # Adjusted to complex for DFT
    F_all = np.zeros_like(img, dtype=np.complex128)  # Store per-channel DFT

    for ch in range(3):
        # Replaced: np.fft.fft2(img, axes=(0,1)) with dft2 (which includes centering)
    ...
    for ch in range(3):
        # Replaced: np.fft.ifft2(compressed_F, axes=(0,1)) with idft2 (expects centered input)
    ...
    # Replaced: Since dft2 already centers, use compressed_F directly (no extra shift needed)
...
def low_pass_filter(img_bgr, radius):
    ...
    F_shifted = np.zeros_like(img, dtype=np.complex128)  # Store centered DFT
    F_masked = np.zeros_like(img, dtype=np.complex128)
    recon = np.zeros_like(img, dtype=np.complex128)

    for ch in range(3):
        # Replaced: np.fft.fft2 + fftshift with dft2 (which computes centered DFT)
    ...
    for ch in range(3):
        # Replaced: np.fft.ifftshift + ifft2 with idft2 (which handles centered input directly)
