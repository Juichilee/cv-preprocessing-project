...
def center_and_normalize(image, scale):
    ...
    minimum, maximum, average, deviation = image_stats(image)
    if deviation == 0:
        raise ValueError("Cannot normalize an array with zero variation")

    t_img = np.copy(image).astype(np.float64)
    for i in range(t_img.shape[0]):
        for j in range(t_img.shape[1]):
            value = t_img[i, j]
            normalized = ((value - average) / deviation) * scale + average
            t_img[i, j] = np.astype(normalized, np.uint8)
...
def shift_image_left(image, shift):
    ...
    if image.ndim != 2:
        raise ValueError("Input must be a two‑dimensional array")
    if not isinstance(shift, int) or shift < 0 or shift >= image.shape[1]:
        raise ValueError("Shift must be an integer between zero and one less than the array width")

    t_img = np.copy(image)
    if shift == 0:
        return t_img

    last_column = np.empty((t_img.shape[0],), dtype=t_img.dtype)

    for row in range(t_img.shape[0]):
        for col in range(t_img.shape[1] - shift):
            t_img[row, col] = t_img[row, col + shift]
            if col == t_img.shape[1] - shift - 1:
                last_column[row] = t_img[row, col]
        for col in range(t_img.shape[1] - shift, t_img.shape[1]):
            t_img[row, col] = last_column[row]
...
def difference_image(img1, img2, output_min=0.0, output_max=255.0):
    ...
    if not (isinstance(output_min, (int, float)) and isinstance(output_max, (int, float))):
        raise ValueError("Output range bounds must be numeric")
    if output_min >= output_max:
        raise ValueError("Output minimum must be less than output maximum")

    a = np.copy(img1).astype(np.float64)
    b = np.copy(img2).astype(np.float64)
    diff = a - b

    minimum = np.min(diff)
    maximum = np.max(diff)
    if minimum == maximum:
        return np.full(diff.shape, output_min)

    normalized = (diff - minimum) / (maximum - minimum)
...
def build_hybrid_image(image1, image2, cutoff_frequency):
    ...
    if image1.ndim != 3 or image2.ndim != 3:
        raise ValueError("Both inputs must be three‑dimensional arrays")
    if image1.shape[2] != 3 or image2.shape[2] != 3:
        raise ValueError("Both inputs must have exactly three channels")
    ...
    img1 = np.copy(image1).astype(np.float64)
    img2 = np.copy(image2).astype(np.float64)
    
    # Create Gaussian kernel for filtering
    kernel_size = 2 * int(4 * cutoff_frequency + 0.5) + 1
    kernel = cv2.getGaussianKernel(kernel_size, cutoff_frequency)
    kernel = np.outer(kernel, kernel)
    
    # Apply filter to each channel of both images
    low_freq = np.zeros_like(img1)
    high_freq = np.zeros_like(img2)
    
    for i in range(3):
        # Get low frequency components from image1
        low_freq[:, :, i] = cv2.filter2D(img1[:, :, i], -1, kernel)
        
        # Get high frequency components from image2
        blurred = cv2.filter2D(img2[:, :, i], -1, kernel)
        high_freq[:, :, i] = img2[:, :, i] - blurred
...
def vis_hybrid_image(hybrid_image, num_scales=5, scale_factor=0.5, padding=5):
    ...
    if hybrid_image.ndim != 3 or hybrid_image.shape[2] != 3:
        raise ValueError("Input must be a three-channel image")
    if hybrid_image.shape[0] <= 0 or hybrid_image.shape[1] <= 0:
    ...
    if not isinstance(scale_factor, (int, float)) or scale_factor <= 0 or scale_factor >= 1:
        raise ValueError("Scale factor must be a number between 0 and 1")
    if not isinstance(padding, int) or padding < 0:
        raise ValueError("Padding must be a non-negative integer")
    
    # Convert to float32 for processing
    img = np.copy(hybrid_image).astype(np.float32)
    
    # Original image height and width
    height = img.shape[0]
    
    # List to store all scaled images
    scaled_images = [img]
    
    # Current image to be scaled
    current_img = img
    ...
    for i in range(1, num_scales):
        ...
        # Resize the image
        resized = cv2.resize(current_img, (new_width, new_height), 
    ...
    total_width = sum(img.shape[1] for img in scaled_images) + padding * (len(scaled_images) - 1)
    
    # Create output canvas
    output = np.zeros((height, total_width, 3), dtype=np.float32)
    
    # Place each scaled image on the canvas
    current_x = 0
    for i, scaled in enumerate(scaled_images):
        # Calculate vertical position to center the image
        y_offset = (height - scaled.shape[0]) // 2
        
        # Place the scaled image
        output[y_offset:y_offset + scaled.shape[0], 
               current_x:current_x + scaled.shape[1], :] = scaled
        
        # Update x position for next image
        current_x += scaled.shape[1] + padding
