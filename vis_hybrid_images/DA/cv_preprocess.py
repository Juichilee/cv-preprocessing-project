...
def build_hybrid_image(image1, image2, cutoff_frequency):
    ...
    if not (isinstance(image1, np.ndarray) and isinstance(image2, np.ndarray)):
        raise ValueError("Both inputs must be NumPy arrays")
    if image1.ndim != 3 or image2.ndim != 3 or image1.shape[2] != 3 or image2.shape[2] != 3:
    ...
    # Compute kernel size (must be odd)
    ksize = 2 * cutoff_frequency + 1

    # Low-frequency: blur first image
    low_freq = cv2.GaussianBlur(img1, (ksize, ksize), cutoff_frequency)

    # High-frequency: original second image minus its blurred version
    blurred_img2 = cv2.GaussianBlur(img2, (ksize, ksize), cutoff_frequency)
...
def vis_hybrid_image(hybrid_image, num_scales=5, scale_factor=0.5, padding=5):
    ...
    if hybrid_image.ndim != 3 or hybrid_image.shape[2] != 3:
        raise ValueError("Input must have exactly three channels")
    if hybrid_image.shape[0] <= 0 or hybrid_image.shape[1] <= 0:
        raise ValueError("Input dimensions must be positive")
    if not isinstance(num_scales, int) or num_scales < 0:
        raise ValueError("Number of scales must be a non-negative integer")
    ...
    original = hybrid_image.astype(np.float32)
    ...
    for _ in range(num_scales):
        ...
        # Resize back to original height for stacking
        resized_full_height = cv2.resize(
            resized,
            (current_width, height),
            interpolation=cv2.INTER_AREA
        )
        images.append(resized_full_height)
    ...
    for i, img in enumerate(images):
        w = img.shape[1]
        output[:, current_x:current_x + w, :] = img
        current_x += w
        if i < len(images) - 1:
            current_x += padding  # Add padding between images
