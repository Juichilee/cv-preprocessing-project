...
def parse_args():
    ...
    parser.add_argument(
        'hybrid_image1',
        help='First image for hybrid combination (part 7)'
    )
    parser.add_argument(
        'hybrid_image2',
        help='Second image for hybrid combination (part 7)'
    )
...
def main():
    ...
    # normalize around zero with fixed scale (part 4b)
    norm = center_and_normalize(green, 10)
    cv2.imwrite('4-b-1.png', norm)

    # shift green image left (part 4c)
    shifted = shift_image_left(green, 2)
    cv2.imwrite('4-c-1.png', shifted)

    # show difference between original and shifted (part 4d)
    diff = difference_image(green, shifted, output_min=0.0, output_max=255.0)
    cv2.imwrite('4-d-1.png', diff.astype(np.uint8))

    # add noise to green channel (part 5a)
    sigma = 20
    noisy_g = add_noise(img1, channel=1, sigma=sigma, mean=0.0)
    cv2.imwrite('5-a-1.png', noisy_g.astype(np.uint8))

    # add noise to blue channel (part 5b)
    noisy_b = add_noise(img1, channel=0, sigma=sigma)
    cv2.imwrite('5-b-1.png', noisy_b.astype(np.uint8))

    # load images for hybrid creation (part 7)
    h1 = cv2.imread(args.hybrid_image1)
    h2 = cv2.imread(args.hybrid_image2)
    if h1 is None or h2 is None:
        raise FileNotFoundError('Could not load one of the hybrid images.')

    # build and save hybrid image
    cutoff = 5
    hybrid = build_hybrid_image(h1, h2, cutoff)
    vis = vis_hybrid_image(hybrid, num_scales=5, scale_factor=0.5, padding=5)
    cv2.imwrite('hybrid_image_scales.png', vis[:, :, ::-1].astype(np.uint8))
