...
def center_and_normalize(image, scale):
    ...
    # Check if standard deviation is non-zero
    if std_dev == 0:
        raise ValueError("Input array must have a non-zero spread of values")
    
    # Apply transformation: ((x - mean) / std_dev) * scale + mean
    normalized = (image - mean) / std_dev * scale + mean
...
def shift_image_left(image, shift):
    ...
    if shift >= width:
        raise ValueError("Shift must be less than the width of the array")
    
    # Optimization: if shift is 0, return the original image
    if shift == 0:
        return image.copy()
    ...
    # For each column, determine its new position
    for col in range(width):
        new_col = (col - shift) % width
        shifted[:, new_col] = image[:, col]
...
def difference_image(img1, img2, output_min=0.0, output_max=255.0):
    ...
    # Ensure output_min is less than output_max
    ...
    # Find min and max of difference
    diff_min = np.min(diff)
    diff_max = np.max(diff)
    
    # Handle case where all differences are the same
    if diff_min == diff_max:
        return np.full_like(diff, (output_min + output_max) / 2)
    
    # Scale the differences to the output range
    scale_factor = (output_max - output_min) / (diff_max - diff_min)
    scaled = (diff - diff_min) * scale_factor + output_min
