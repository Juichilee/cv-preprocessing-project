...
def center_and_normalize(image, scale):
    ...
    img_copy = np.copy(image).astype(np.float64)
    mean_val = np.mean(img_copy)
    std_val = np.std(img_copy)
    
    if std_val == 0:
        raise ValueError("Input image has zero standard deviation; cannot normalize")
    
    # Center around zero, scale to target std, then shift back to original mean
    normalized = (img_copy - mean_val) / std_val * scale + mean_val
    return normalized
...
def shift_image_left(image, shift):
    ...
    if not isinstance(shift, int) or shift < 0 or shift >= image.shape[1]:
        raise ValueError("Shift must be a non-negative integer less than the image width")

    result = np.copy(image)
    if shift > 0:  # Only shift if non-zero
        result[:, :-shift] = image[:, shift:]  # Shift left
        result[:, -shift:] = image[:, :shift]  # Wrap around leftmost columns
    return result
...
def difference_image(img1, img2, output_min=0.0, output_max=255.0):
    ...
    if not isinstance(output_min, (int, float)) or not isinstance(output_max, (int, float)):
        raise ValueError("Output min and max must be numeric")
    ...
    if diff_min == diff_max:
        # If all differences are the same, return a constant array at output_min
        return np.full_like(diff, output_min)
    
    # Scale to [output_min, output_max]
    scaled = (diff - diff_min) / (diff_max - diff_min) * (output_max - output_min) + output_min
    return scaled
