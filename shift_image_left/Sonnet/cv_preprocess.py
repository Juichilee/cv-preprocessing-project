...
def center_and_normalize(image, scale):
    ...
    result = (result - mean_val) * (scale / std_val) + mean_val
...
def shift_image_left(image, shift):
    ...
    if shift >= image.shape[1]:
        raise ValueError("Shift amount must be less than the width of the array")
    ...
    # Store the columns that would fall off the left edge
    left_columns = result[:, :shift]
    
    # Shift the main part of the image
    result[:, :-shift] = result[:, shift:]
    
    # Fill the vacant spots on the right with the values from the left edge
    result[:, -shift:] = left_columns
...
def difference_image(img1, img2, output_min=0.0, output_max=255.0):
    ...
    if not (isinstance(output_min, (int, float)) and isinstance(output_max, (int, float))):
        raise ValueError("Output range limits must be numeric")
    if output_min >= output_max:
        raise ValueError("Output minimum must be less than output maximum")
    
    # Calculate the difference
    diff = np.copy(img1).astype(np.float64) - np.copy(img2).astype(np.float64)
    
    # Find the min and max of the difference
    diff_min = np.min(diff)
    diff_max = np.max(diff)
    
    # Handle the case where all differences are the same
    if diff_min == diff_max:
        return np.ones_like(diff) * output_min
    
    # Scale the difference to the output range
    scaled_diff = (diff - diff_min) / (diff_max - diff_min) * (output_max - output_min) + output_min
