...
def copy_paste_middle(src, dst, shape):
    ...
    if not isinstance(src, np.ndarray) or not isinstance(dst, np.ndarray):
        raise ValueError("Source and destination must be NumPy arrays")
    ...
    # Check if the block fits in the source
    if height > src.shape[0] or width > src.shape[1]:
        raise ValueError("Block size exceeds source dimensions")
    
    # Check if the block fits in the destination
    if height > dst.shape[0] or width > dst.shape[1]:
        raise ValueError("Block size exceeds destination dimensions")
    
    # Create a copy of the destination
    result = np.copy(dst)
    
    # Calculate center coordinates for source and destination
    src_center_y = src.shape[0] // 2
    src_center_x = src.shape[1] // 2
    dst_center_y = dst.shape[0] // 2
    dst_center_x = dst.shape[1] // 2
    
    # Calculate the top-left corner of the block in source and destination
    src_start_y = src_center_y - height // 2
    src_start_x = src_center_x - width // 2
    dst_start_y = dst_center_y - height // 2
    dst_start_x = dst_center_x - width // 2
    
    # Extract the block from source and paste it into destination
    src_block = src[src_start_y:src_start_y + height, src_start_x:src_start_x + width]
    result[dst_start_y:dst_start_y + height, dst_start_x:dst_start_x + width] = src_block
...
def copy_paste_middle_circle(src, dst, radius):
    ...
    # Calculate center coordinates for source and destination
    src_center_y = (src.shape[0] - 1) // 2
    src_center_x = (src.shape[1] - 1) // 2
    dst_center_y = (dst.shape[0] - 1) // 2
    dst_center_x = (dst.shape[1] - 1) // 2
    
    # Create coordinate grids for the source
    y_coords, x_coords = np.ogrid[:src.shape[0], :src.shape[1]]
    
    # Calculate distances from the center for each pixel in source
    distances = np.sqrt((y_coords - src_center_y)**2 + (x_coords - src_center_x)**2)
    
    # Create a mask for pixels within the radius
    mask = distances <= radius
    
    # Get the coordinates of pixels within the circle
    y_indices, x_indices = np.where(mask)
    
    # Calculate the offsets from the center
    y_offsets = y_indices - src_center_y
    x_offsets = x_indices - src_center_x
    
    # Calculate the corresponding destination coordinates
    dst_y = dst_center_y + y_offsets
    dst_x = dst_center_x + x_offsets
    
    # Filter out coordinates that would fall outside the destination
    valid_indices = (
        (dst_y >= 0) & (dst_y < dst.shape[0]) & 
        (dst_x >= 0) & (dst_x < dst.shape[1])
    )
    
    # Copy the values from source to destination
    result[dst_y[valid_indices], dst_x[valid_indices]] = src[y_indices[valid_indices], x_indices[valid_indices]]
